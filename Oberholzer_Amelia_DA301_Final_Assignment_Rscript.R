---
title: "Oberholzer_Amelia_DA301-Assignment_markdown_Rscript.R"
output:
...
Here most of the products have sales in between 0 and 5 million however some products have sold exceptionally well. I will go ahead to identify top selling products as well as products that havent sold as well. 
...
```
We see here how the bottom and top plots compare. Interestingly PC have the lowest sales performance. Whereas Will, NES and GB have higher sales performance. 
You can also see the individual product numbers. Turtle Games can use their directory to identify which product corresponds to the number to gain more information.
It is interesting to see how some platforms are more popular in different regions. I will now go on to investigate this! 
```{r}
# Use group by function() to get the sum
# Of sales for certain platform 
...
```
I will now filter out the highest platform sales in EU and NA for a clear comparison
```{r}
# First EU 
...
```
Interestingly we see how the Wii is more popular in the EU whereas the Xbox 360 is more popular in the US. This information can information Turtle Games on where to focus marketing when new products have been released from these platforms. 
Now were going to observe how sales change over time using group_by to find the total global sales per year. 
...
head(predictTest)
```
Here we can make comparison of the accuracy of the model. If NA_Sales_sum is 34.02 and EU_Sales_sum is 23.80 the model predicts global sales will be 72.77. This is not too far off the value of 67.84 the actual value 
You can also see the lower and upper values defining the confidence interval in our predicted values. The confidence interval in the predict function 
